...
# Имя у страницы ОДНО и то же везде: заголовок, крошки, бланк, карточки,
# подвал. Была попытка держать отдельную короткую подпись для узких мест, но
# замер показал, что длинные H1 везде помещаются в одну строку, а живая
# проверка показала худшее: по сайту разошлись три имени одной страницы, и
# человек, кликнувший «ПМЖ по инвестициям», попадал на «ПМЖ за недвижимость».
