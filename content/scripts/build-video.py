...
Петля замкнута с двух сторон, и это разные вещи. По СОДЕРЖАНИЮ — тем, что
в конец списка дописан первый исходник. По МАСШТАБУ — тем, что наезд идёт
по косинусу туда и обратно. Линейный наезд замер не ловит вообще: tblend
считает разницу соседних кадров ВНУТРИ файла, а щелчок стоит на стыке
цикла, где масштаб 1.16 сменяется на 1.00. У видео на странице `loop`,
значит этот щелчок повторялся бы каждые двадцать две секунды.
"""
import math
import pathlib
import shutil
...
# Выдержка. Поток гасит переход между исходниками не до конца: настоящий
# исходный кадр «встаёт на место» рывком, и в готовом файле это стояло
# пиками каждые 40 кадров, то есть ровно на стыках, 9.28 при фоне 3.72.
# Усреднение трёх соседних кадров весами 1-2-1 это те же 33 мс смаза,
# что даёт затвор настоящей камеры. Замер после: фон 2.52, пик 5.53.
#
# tmix копит окно и выдаёт среднее ПОСЛЕДНИХ трёх кадров, то есть отстаёт
# на кадр, а первые два выхода усредняет по укороченному окну. Поэтому на
# вход дописаны кадры-обёртки с той стороны петли (см. encode), а первые
# два выхода отрезаны: тогда окно замкнуто и на стыке цикла тоже.
...
def travel(i: int, n: int) -> float:
    """Ход наезда для кадра i из n: 0 → 1 → 0 по косинусу.

    Не i/(n-1). Линейный ход красивее по ощущению, но у него конец не равен
    началу, а видео на странице зациклено: масштаб щёлкал бы обратно каждую
    петлю. По косинусу и края совпадают, и скорость на них нулевая, поэтому
    разворот движения тоже не читается.
    """
...
def encode(name: str, w: int, h: int, crf: int,
    ...
    try:
        n = len(frames)
        # Порядок на входе: [последний, весь полёт, первый]. Обёртки нужны
        # окну выдержки: без них первый кадр видео усреднялся бы по обрезку
        # окна и отличался бы от того, во что перетекает последний.
        order = [(n - 1, frames[-1])] + list(enumerate(frames)) + [(0, frames[0])]
        for pos, (i, src) in enumerate(order, start=1):
            dress(src, travel(i, n), w, h).save(out / f"p_{pos:04d}.png")
