...
TARGETS = [
    # CRF 28, а не 25. Кадр вырос с 960x400 до 1280x534, чтобы браузер меньше
    # растягивал его в бокс героя, и при прежнем CRF файл потяжелел с 2.42 до
    # 3.89 МБ. Это лечило бы мыло ценой скорости: ролик и так докачивался
    # двадцать секунд. Более крупный кадр при более высоком CRF даёт то же
    # разрешение примерно за прежний вес. Сцена мягкая и зернистая, потери
    # CRF тут дешевле потерь разрешения.
    ("cyprus-descent.mp4", bf.OUT_W, bf.OUT_H, 28, "poster.avif"),
