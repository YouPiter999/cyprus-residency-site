...
def compose(base: list[Image.Image], k: int, u: float) -> Image.Image:
    ...
    У шага k=0 предыдущая сцена берётся с конца списка (`base[-1]`), и это
    не мелочь, а стык петли. Ролик висит с `loop`, значит после последней
    сцены зритель снова видит первую, и раньше эта единственная смена шла
    БЕЗ перехвата: под водой щёлкало обратно в облака. Отдельного лечения
    вроде затемнения на стыке не нужно — нужно, чтобы стык петли работал
    ровно так же, как остальные четырнадцать смен.
    """
    s = ZOOM_STEP ** u
...
def encode(name: str, w: int, h: int, crf: int, base: list[Image.Image],
    ...
    try:
        ...
        # Раскладка замкнута в круг, поэтому важно, с какого места круга
        # начинается ФАЙЛ. В самом начале шага k=0 идёт перехват, и там ещё
        # видна уходящая последняя сцена: поставь файл начинаться отсюда —
        # и страница откроется мутной водой вместо облаков, да и постер
        # (первый кадр этого же ролика) выйдет такой же. Проворачиваем круг
        # на длину перехвата: файл начинается с первой сцены начисто, а
        # перехват из последней в первую уезжает в самый хвост, где его и
        # подхватывает петля.
        off = int(round(per_step * FADE))
        cycle = [(i // per_step, (i % per_step) / per_step)
                 for i in range(total)]
        cycle = cycle[off:] + cycle[:off]
        seq = [cycle[0]] + cycle + [cycle[-1]]
        ...
        # видна подмена картинки ещё до того, как что-то поедет. Круг
        # провёрнут, так что первый кадр это cycle[0], а не (0, 0.0)
        pdst = ROOT / poster
        dress(compose(base, *cycle[0]), w, h).save(pdst, "AVIF", quality=62)
